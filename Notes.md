# Day 1 – Python Practice Set

## Topics Covered
- Printed poem using print()
- Used REPL to print table of 5
- Installed and used external module (pyjokes)
- Used os module to list directory contents
- Added comments in code

## Key Learnings
- Basic use of print and REPL
- How to install and import modules
- os.listdir() shows files in a directory
- Importance of comments

## Issue Faced
- ModuleNotFoundError due to different Python versions


# Day 2 – Python Practice Set

## Topics Covered
- Added two numbers
- Found remainder using modulus (%)
- Took user input using input()
- Checked variable type using type()
- Used comparison operators (>, <)
- Calculated average of two numbers
- Calculated square of a number

## Key Learnings
- input() takes user input as string (needs type conversion)
- Used int() and float() for conversion
- % operator gives remainder
- Comparison operators return True/False

## Issue Faced
- Understood type conversion is important when using input()

# Day 3 – Python Practice Set

## Topics Covered
- Took user input and displayed message
- Used string formatting for letter template
- Detected double spaces in a string
- Replaced double spaces with single space
- Used escape sequences (\n, \t) for formatting

## Key Learnings
- Strings can be modified using replace()
- in keyword helps to check substring
- Escape sequences improve output formatting
- Input can be used inside strings

## Issue Faced
- Handling spaces in strings and formatting output